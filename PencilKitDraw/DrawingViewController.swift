...
public func getDocumentDirectory() -> URL {
    // https://www.youtube.com/watch?v=DCeW2TPtzTE&t=24s
...
//        canvasView.tool = PKInkingTool(.pen, color: .black, width: 20)
    ...
    func updateContentSizeForDrawing() {
        ...
        print("updateContentSizeForDrawing")
        ...
        if(strokeIndex > -1)
        {
            let strokeToAnimate = canvasView.drawing.strokes[strokeIndex]
            print("start pose")
//            print(strokeToAnimate.path.interpolatedLocation(at: 0).applying(strokeToAnimate.transform))
            
            let adjustedStrokes = canvasView.drawing.strokes.map { stroke -> PKStroke in
                var stroke = stroke
...
//                stroke.ink = PKInk(.pen, color: TextGenerator.templateColor)
            ...
            print("adjustedStrokes")
//            print(adjustedStrokes[strokeIndex].path.count)
            let pointCount = adjustedStrokes[strokeIndex].path.count
            var x = 0.0
            var y = 0.0
            var force = 0.0

            var filename = "traj.csv"
            let url = getDocumentDirectory().appendingPathComponent(filename)
            
            print("url: ", url)
            
            let res: Bool = FileManager.default.createFile(atPath: url.path, contents: nil)
            if !res {
                        print("FileManager cannot create the file \(url.path). ")
            }
            let stream = OutputStream(url:url, append: false)!
            let csv = try! CSVWriter(stream: stream)

            try! csv.write(row: ["x", "y", "force"])
 
            for idx in 0...(pointCount-1){
                x = adjustedStrokes[strokeIndex].path[idx].location.x
                y = adjustedStrokes[strokeIndex].path[idx].location.y
                force = adjustedStrokes[strokeIndex].path[idx].force
//                print( "x: ",x, " y: ",y," f: ",force)
                try! csv.write(row:[String(x), String(y), String(force)])
//                try! csv.write(row: [NSString(format: "%.2f", x), NSString(format: "%.2f", y), NSString(format: "%.2f", force)])
            }
            csv.stream.close()
        }
